...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (profileFile) {
        try {
          ...
          if (!uploadResponse.ok) {
            ...
            setError(errorData.error || "Failed to upload profile image.");
            setLoading(false);
            return;
          }
      ...
      // The AuthContext signup function will handle the page refresh and navigation
    } catch (err) {
    ...
    >
      <div className="w-full flex justify-end p-4">
        <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
          <Link to="/provider/signup" className="text-sm text-puce hover:underline">
      ...
      <motion.h1
        ...
        custom={1}
      >
        Sign up to look for service<br className="hidden sm:block" /> providers / Salons
      </motion.h1>

      <motion.div className="flex flex-col sm:flex-row gap-3 w-full max-w-md mx-auto mb-4" variants={fadeInUp} custom={2}>
        <button type="button" className="flex-1 flex items-center justify-center gap-2 border border-gray-200 rounded-full py-2 font-medium hover:border-puce text-black transition">
      ...
      <motion.form className="w-full max-w-2xl mx-auto space-y-4" variants={fadeInUp} custom={5} onSubmit={handleSubmit}>
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 flex flex-col text-gray-900 text-sm">
            ...
            <input type="text" placeholder="Type here..." className="border border-gray-200 rounded-lg py-2 px-3 text-sm focus:outline-none focus:border-puce transition"
          ...
          <div className="flex-1 flex flex-col text-gray-900 text-sm">
            ...
            <div className="relative">
              ...
              <input type="email" placeholder="Enter email address..." className="w-full border border-gray-200 rounded-lg py-2 pl-10 pr-3 text-sm focus:outline-none focus:border-puce transition"
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 flex flex-col text-gray-900 text-sm">
            ...
            <div className="relative">
              <input type={showPassword ? "text" : "password"} placeholder="Type here..." className="w-full border border-gray-200 rounded-lg py-2 px-3 text-sm focus:outline-none focus:border-puce transition"
                value={password} onChange={(e) => setPassword(e.target.value)} aria-label="Password" />
              <button type="button" tabIndex={-1} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400"
          ...
          <div className="flex-1 flex flex-col text-gray-900 text-sm">
            ...
            <div className="relative">
              <input type={showConfirm ? "text" : "password"} placeholder="Type here..." className="w-full border border-gray-200 rounded-lg py-2 px-3 text-sm focus:outline-none focus:border-puce transition"
                value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} aria-label="Confirm Password" />
              <button type="button" tabIndex={-1} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400"
        ...
        <div>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-2">
            <div className="flex-1 flex flex-col text-gray-900 text-sm">
              ...
              <select className="border border-gray-200 rounded-lg py-2 px-3 text-sm focus:outline-none focus:border-puce transition"
            ...
            <div className="flex-1 flex flex-col text-gray-900 text-sm">
              ...
              <input type="text" placeholder="Type here..." className="border border-gray-200 rounded-lg py-2 px-3 text-sm focus:outline-none focus:border-puce transition"
                aria-label="City/District" value={city} onChange={(e) => setCity(e.target.value)} />
            </div>
            <div className="flex-1 flex flex-col text-gray-900 text-sm">
              <label className="mb-1">Post Code</label>
              <input type="text" placeholder="Type here..." className="border border-gray-200 rounded-lg py-2 px-3 text-sm focus:outline-none focus:border-puce transition"
                aria-label="Postcode" value={postcode} onChange={(e) => setPostcode(e.target.value)} />
            </div>
          </div>
          <div className="flex flex-col text-gray-900 text-sm">
            <label className="mb-1">Full Address</label>
            <input type="text" placeholder="Type here..." className="w-full border border-gray-200 rounded-lg py-2 px-3 text-sm focus:outline-none focus:border-puce transition"
              aria-label="Full Address" value={fullAddress} onChange={(e) => setFullAddress(e.target.value)} />
          </div>
        </div>

        <motion.button className="w-full bg-puce hover:bg-puce1-600 text-white font-semibold py-3 rounded-lg transition mt-4"
          type="submit" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} disabled={loading}>
        ...
        <div className="text-center text-sm text-gray-700 mt-3">
          ...
          <Link to="/login" className="text-puce underline hover:text-puce1-600">Log in</Link>
        </div>
      </motion.form>
    </motion.div>
  );
