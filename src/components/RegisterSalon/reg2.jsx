...
export default function RegisterSalonForm() {
  ...
  const handleAddAddress = () => {
    setAddresses(prev => [...prev, { country: "", city: "", postcode: "", full_address: "" }]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://beautysalon-qq6r.vercel.app/api/salons", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          phone,
          description,
          type: "salon_owner", // or other type if selectable
          addresses,
      ...
      if (!response.ok) {
        ...
        // Salon created successfully - redirect to dashboard
        const salonId = data.salon_id;
        // Store salonId in localStorage for dashboard access
        localStorage.setItem('salonId', salonId);
        // Navigate to dashboard with salonId
