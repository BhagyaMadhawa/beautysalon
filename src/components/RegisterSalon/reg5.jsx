...
export default function OperatingHoursStep() {
  ...
  const handleToggle = idx => {
    setHours(prev =>
      prev.map((d, i) =>
        i === idx ? { ...d, is_opened: !d.is_opened } : d
      )
    );
  };
  ...
  return (
    ...
    >
            ...
            {idx < stepLabels.length - 1 && (
              <div
                className={`h-1 rounded
                  ${idx < 3 ? "bg-rose-taupe" : "bg-gray-200"}
                  w-8 sm:w-10`}
              />
            )}
          </React.Fragment>
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-full sm:max-w-[90%] md:max-w-[75%] bg-white p-2 sm:p-4 md:p-8 mb-8">
        <div className="mb-2">
        ...
        <div className="divide-y">
          {hours.map((row, idx) => (
            <div
              key={row.day}
              className="flex flex-col sm:flex-row items-center gap-y-4 sm:gap-x-20 py-4 sm:py-6"
            >
              {/* Day Label */}
              ...
              <label className="relative inline-flex items-center cursor-pointer mr-2">
                ...
                />
                <div
                  className="w-10 h-6 bg-gray-200 rounded-full peer peer-checked:bg-puce1-500 transition-colors duration-200"
                ></div>
                <div
                  className="absolute left-0.5 top-0.5 w-5 h-5 bg-white border border-gray-400 rounded-full shadow
